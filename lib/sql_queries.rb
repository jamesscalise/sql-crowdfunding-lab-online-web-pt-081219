...
def selects_the_titles_and_amount_over_goal_of_all_projects_that_have_met_their_funding_goal
  "SELECT projects.title, (SUM(pledges.amount) - projects.funding_goal) AS amount_left FROM projects INNER JOIN pledges ON projects.id = pledges.project_id WHERE (SUM(pledges.amount) >= projects.funding_goal) GROUP BY projects.title HAVING amount_left >= 0"
end
